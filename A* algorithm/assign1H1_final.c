...
Board15* optimalBoard1(){
    ...
    while(listhead->right!=NULL){
        ...
        for(i=0;i<15;i++){
            ...
            if(listhead->board->blank!=i){
                if(listhead->board->b[i]!=i+1){
        ...
        if(f_min==-1){
            ...
            f_min = count + listhead->board->cost;
            optBoard = listhead->board;
        }
        else if(f_min > count + listhead->board->cost){
...
Board15* solvePuzzle15(Board15* board, int* time_comp, int* space_comp){
...
int main(){
    ...
    if(checkFinalState(board)==1){
        ...
        *time_comp=0;
        *space_comp=0;
        Board15* finalboard=solvePuzzle15(board,time_comp,space_comp);
        if(finalboard!=NULL){
